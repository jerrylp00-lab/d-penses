...
_NOISE = {"loyer", "rent", "house", "location", "garage", "val", "labbe", "romane",
          "caution", "vir", "inst", "de", "le", "la", "les", "du", "des", "une",
          "monsieur", "madame", "mme", "mr", "mme", "et", "pi", "pour"}

def _name_key(label: str) -> str:
    """Extract 1-2 significant words from a label for person matching."""
    cleaned = re.sub(r'\b(VIR INST de|VIR INST DE|DE M\.|DE MR|DE MME|M\.|MR|MME)\b', '', label, flags=re.IGNORECASE)
    words = [w for w in re.split(r'\W+', cleaned) if len(w) >= 3 and w.lower() not in _NOISE]
    return ' '.join(words[:2]).upper()


def _build_tenant_history(loyer_txs: list[dict], all_txs: list[dict], loyer_montant: float, caution_attendue: int) -> list[dict]:
    """
    Group loyer transactions by tenant (name key), find their caution payments and returns.
    Returns list of tenants sorted by most recent first.
    """
    # Group loyer txs by tenant key
    groups: dict[str, list[dict]] = {}
    for tx in loyer_txs:
        key = _name_key(tx["label"])
        if not key:
            key = "INCONNU"
    ...
    for key, txs in groups.items():
        txs_sorted = _sorted_desc(txs)
        first_date = min(t["date"] for t in txs)
        last_date  = max(t["date"] for t in txs)

        # Find significant words (>=4 chars) from this key for broader matching
        sig_words = [w for w in key.split() if len(w) >= 4]

        caution_recue    = []
        caution_rendue   = []

        for tx in all_txs:
            if abs(tx["amount"] - loyer_montant) < 1:
                continue  # skip regular loyer
            label_up = tx["label"].upper()
            if not any(w in label_up for w in sig_words):
                continue
            if tx["amount"] > 0:
                caution_recue.append(tx)
            elif tx["amount"] < 0:
                caution_rendue.append(tx)

        total_recu  = round(sum(t["amount"] for t in caution_recue), 2)
        total_rendu = round(abs(sum(t["amount"] for t in caution_rendue)), 2)

        tenants.append({
            "name":             key.title(),
            "first_date":       first_date,
            "last_date":        last_date,
            "loyer_txs":        txs_sorted,
            "caution_attendue": caution_attendue,
            "caution_recue":    _sorted_desc(caution_recue),
            "caution_rendue":   _sorted_desc(caution_rendue),
            "total_recu":       total_recu,
            "total_rendu":      total_rendu,
            "caution_complete": total_recu >= caution_attendue,
        })

    # Sort: current tenant first (most recent last_date)
    tenants.sort(key=lambda t: t["last_date"], reverse=True)
    return tenants
...
def _build_dashboard_data(transactions: list[dict]) -> dict:
    ...
    recettes_txs  = _sorted_desc([t for t in transactions if t["amount"] > 0 and t["date"][:7] == now_ym])
    depenses_txs  = _sorted_desc([t for t in transactions if t["amount"] < 0 and t["date"][:7] == now_ym])
    recettes_mois = sum(t["amount"] for t in recettes_txs)
    depenses_mois = abs(sum(t["amount"] for t in depenses_txs))

    loyer_cards = []
    for loyer_cfg in config.LOYERS:
        montant = loyer_cfg["montant"]
        caution = loyer_cfg.get("caution", 0)
        matching = _sorted_desc([
            t for t in transactions
            if t["category"] == "loyer" and abs(t["amount"] - montant) < 5
        ])
        last_tx = {"date": matching[0]["date"], "amount": matching[0]["amount"]} if matching else None
        tenants = _build_tenant_history(matching, transactions, montant, caution) if matching else []
        loyer_cards.append({
            "appart":          loyer_cfg["appart"],
            "type":            loyer_cfg["type"],
            "montant_attendu": montant,
            "caution_attendue": caution,
            "last_tx":         last_tx,
            "all_txs":         matching,
            "tenants":         tenants,
        })
    ...
    return {
        ...
        "recettes_txs":            recettes_txs,
        "depenses_mois":           round(depenses_mois, 2),
        "depenses_txs":            depenses_txs,
